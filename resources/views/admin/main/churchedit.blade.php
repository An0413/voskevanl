    ...
    <div class="content-wrapper">
        ...
        <div class="content-header">
            <div class="container-fluid">
                ...
                <form action="{{ route('church_update', $church->id) }}" method="post" enctype="multipart/form-data">
                    @csrf
                    <div class="mb-3 mt-3 w-50">
                        <label for="title">Վերնագիր</label>
                        <input type="text" class="form-control" name="title" value="{{ $church->title }}">
                    </div>
                    <div class="mb-3 mt-3 w-50">
                        <label for="description">Նկարագրություն</label>
                        <textarea class="form-control" rows="5" id="comment" name="description"
                        >{{ $church->description }}</textarea>
                </div>
                    <div>
                        <img src="{{asset('assets/img/church/'.$church->img) }}" style="width: 150px; height: 150px;">
                    </div>
                <div>
